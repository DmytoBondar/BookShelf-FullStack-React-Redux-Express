...
const Books = ({ book }: IProps) => {
    ...
    return (
        <div className="w-full bg-gradient-to-r mb-3 from-cyan-200 to-blue-100 w-120 h-60 rounded shadow-md flex card text-grey-darkest">
            <Link to={`/book/${book._id}`} className="w-1/2 bg-gray-300">
                <img className="h-full rounded-l-sm mx-auto" src={book.image} alt="Room Image" />
            </Link>
            <div className="w-1/2 flex flex-col">
                <div className="p-4 pb-0 flex-1">
                    <h2 className="font-bold mb-1 text-grey-darkest truncate block">{book.title}</h2>
                    ...
                    <div className="text-xs flex items-center mb-4">
                        Publication Date : {new Date(book?.publicationDate as string).toLocaleString() || null || undefined}
                    </div>
                </div>
                {
                    user && <div className="bg-lime-700 p-3 flex items-center justify-between transition hover:bg-grey-light">
